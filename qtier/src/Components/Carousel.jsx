import React, { useState,useEffect } from "react";
import { useSpring, animated } from "@react-spring/web";
import { Link } from "react-router-dom";

import Aos from 'aos'
import 'aos/dist/aos.css'

const products = [
  { 
    id: 1, 
    title: "N1 Lux", 
    description: "A stylish single sofa from the N1 Collection, designed to add a modern touch to any space.", 
    imageUrl: "https://i.pinimg.com/736x/61/66/8a/61668a9ff40e12b084fba4d4b012de39.jpg", 
    collection: "N1 Collection"
  },
  { 
    id: 2, 
    title: "N2 Luxe", 
    description: "A sophisticated piece from the N2 Collection, offering both comfort and elegance.", 
    imageUrl: "https://i.pinimg.com/736x/41/22/e3/4122e3911070698adc2716932059ee13.jpg", 
    collection: "N2 Collection"
  },
  { 
    id: 3, 
    title: "N3 Comfort", 
    description: "Combining modern design and comfort, this sofa from the N3 Collection is a perfect addition.", 
    imageUrl: "https://i.pinimg.com/736x/59/bb/4c/59bb4c1b47625717fcdda051e7f867b5.jpg", 
    collection: "N3 Collection"
  },
  { 
    id: 4, 
    title: "N4 Chic", 
    description: "A blend of elegance and relaxation, the N4 Collection sofa is designed for modern living.", 
    imageUrl: "https://i.pinimg.com/736x/40/88/39/408839a7e2b5b27ed31161202865b3b9.jpg", 
    collection: "N4 Collection"
  },
  { 
    id: 5, 
    title: "Pietra Sofa", 
    description: "From the Pietra Collection, a single sofa offering minimalist design and ultimate comfort.", 
    imageUrl: "https://i.pinimg.com/736x/f6/9d/80/f69d80b559f8d6898fcf0b2d86653ba1.jpg", 
    collection: "Pietra Collection"
  },
  { 
    id: 6, 
    title: "Outdoor Comfort", 
    description: "Perfect for outdoor spaces, this sofa combines durability with style, from the Outdoor Collection.", 
    imageUrl: "https://i.pinimg.com/736x/21/60/42/2160422af1a269504f0fc224375d6bb5.jpg", 
    collection: "Shop Outdoor"
  },
  { 
    id: 7, 
    title: "Lounge Retreat", 
    description: "A cozy lounge chair from the Lounge Chairs collection, designed for ultimate relaxation.", 
    imageUrl: "https://i.pinimg.com/736x/72/36/3e/72363e89c17f3b8336183a90c01995fc.jpg", 
    collection: "Lounge Chairs"
  },
  { 
    id: 8, 
    title: "Dine ", 
    description: "A versatile side chair from the Side Tables Collection, perfect for any corner.", 
    imageUrl: "https://i.pinimg.com/736x/f8/0e/85/f80e852b80a7fedcbc0fe5f16044e4dd.jpg", 
    collection: "Side Tables"
  },
  { 
    id: 9, 
    title: "Sofa Set Supreme", 
    description: "An elegant and spacious sofa set from the Sofa Sets Collection, designed for modern living.", 
    imageUrl: "https://i.pinimg.com/736x/58/f4/39/58f439b2c6513f96ccef558ec4d87288.jpg", 
    collection: "Sofa Sets"
  },
  { 
    id: 10, 
    title: " NightStands", 
    description: "From the NightStands Collection, this stand combines comfort and style for dining areas.", 
    imageUrl: "https://i.pinimg.com/736x/fa/e4/be/fae4be7d2144f040a74d703d24e657fa.jpg", 
    collection: "Dining Tables & Chairs"
  },
  { 
    id: 11, 
    title: "Sun Lounger", 
    description: "A stylish sun lounger from the Sun Loungers Collection, perfect for your garden or poolside.", 
    imageUrl: "https://i.pinimg.com/736x/0e/d4/cf/0ed4cf8adbeb91023185132a438bdf0f.jpg", 
    collection: "Sun Loungers"
  },
  { 
    id: 12, 
    title: "Coffee Ulta", 
    description: "A unique blend of coffee table and sofa, perfect for your living room.", 
    imageUrl: "https://i.pinimg.com/736x/33/6f/9b/336f9bb7c71f56a9bb7593a3bfff7df1.jpg", 
    collection: "Coffee Tables"
  },
];


const Carousel = () => {
    const [index, setIndex] = useState(0);

    useEffect(()=>{
      Aos.init();
        },[])
    const slideAnimation = useSpring({
      transform: `translateX(-${index * 100}%)`,
      config: { tension: 180, friction: 20 },
    });
  
    const nextSlide = () => {
      setIndex((prevIndex) => (prevIndex + 1) % Math.ceil(products.length / 4));
    };
  
    const prevSlide = () => {
      setIndex((prevIndex) => (prevIndex - 1 + Math.ceil(products.length / 4)) % Math.ceil(products.length / 4));
    };
  return (
    <div className="relative w-full overflow-hidden">
       <div class="flex flex-col text-left px-7 w-full ">
      <h1 class="text-xl md:text-5xl  text-stone-700 yeseva">SHOP</h1>
      <p class="lg:w-2/3  leading-relaxed text-sm md:text-xl ">Most Favourites!</p>
    </div>
      <animated.div
        className="flex transition-transform duration-500 my-16 mx-auto px-2 container space-x-4 md:space-x-2"
        style={slideAnimation}
      >
        {products.map((product, idx) => (
          <div
            key={product.id}
            className="flex-shrink-0 w-full h-72 md:w-1/2 lg:w-1/4 p-2 rounded-md relative"  
          >
            <Link to={'/collection/1'} className="block">
              <img
                src={product.imageUrl}
                alt={product.title}
                className="w-full h-72 absolute object-center  inset-0 object-cover"
              /><div class="px-4 py-1  relative z-10 w-1/2   bg-white ">
              <h2 class=" text-xs  font-light text-zinc-500 ">Available now</h2>
            </div> <div class="px-8 py-4 mt-5 relative z-10 w-full border-4 border-zinc-700 bg-white bg-opacity-75  opacity-0 hover:opacity-100">
            
            <h2 class=" text-xs  font-light text-zinc-500 ">{product.collection}</h2>
            <h1 class=" text-lg font-light yeseva text-stone-900 my-3">{product.title} </h1>
            <p class="text-sm">{product.description}</p>
          </div>
                   </Link>
          </div>
        ))}
        
      </animated.div>

      {/* Navigation buttons */}
      <button
        onClick={prevSlide}
        className="absolute left-0 top-1/2 transform -translate-y-1/2 text-white text-xl font-bold bg-black p-2"
      >
        &#8249;
      </button>
      <button
        onClick={nextSlide}
        className="absolute right-0 top-1/2 transform -translate-y-1/2 text-white text-xl font-bold bg-black p-2"
      >
        &#8250;
      </button>
    </div>
  );
};

export default Carousel;
